...
- ✅ **Two different colour strategies for our injected UI, by intent:**
  - **Exact colours that must NOT be DR-converted** (the 4 palette swatches): inline `!important` + strip `data-darkreader-inline-*` + a small `MutationObserver` to re-assert. The swatches also re-assert via `applySettings` so they track palette changes.
  - **Theme-matching colours** (Easy Shade button text/glow, the opacity card): **reference SudokuPad's live CSS variable**, the way the native buttons do, and let DR convert it. Do NOT snapshot a computed colour (`getComputedStyle(...).color`) at build time and force it — that value races DR's conversion and the result **flickers white↔purple** per load. Forcing our own card colours with `!important` + an observer made it worse (we fought DR mid-convert); removed in v2.126, now we just let DR handle the card like any native dark popup.
- ⚠️ **DR rewrites `var(--x)` usage inside *stylesheets* (to the `--darkreader-text--x` / `--darkreader-bg--x` variant per property), but NOT inline `style` var usage.** So an inline `color: var(--controls-button-bg)` resolves to the raw **background** purple (`#6a1b9a`, too dark for text), not the bright text variant. For inline foreground use, reference the converted var directly with fallbacks: `var(--darkreader-text--controls-button-bg, var(--controls-button-bg, #b568e4))` → bright `#b568e4`, stable.
