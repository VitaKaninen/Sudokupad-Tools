...
- **Cell shading** + **Object shading / underlay** — opacity / brightness controls. Object shading has a **Brightness-mode** selector (`underlayBrightnessModel`, v2.130): `absolute` (default — pure hue at HSL lightness = slider), `relative` (keep hue+sat+relative lightness; 0.5 = original), `luminance` (pick lightness so *perceived* luma = slider, so all hues read equally bright). It also shades coloured **`#overlay` shapes** (lucky-charm circles/squares/diamonds), not just `#underlay` — opt-in via `underlayOverlayEnabled` (default on), with a broad skip-rule (`shouldShadeOverlayRect`: skip `.textbg`, transparent, and white/near-white rects).
