...
- **Validate Constraints — the validator subsystem (v3.53+; full documentation in
  ...
  *stale* (kept on screen, no longer trusted as input — `validatorHiliteRuledOut` vs
  ...
  forward-only pass, and a manual highlight run kicks it too via `scheduleAutoValidators`;
  **v3.150** stops a re-run trusting its OWN flags — `validatorHiliteSuppressOwn` — without which
  the re-run finds nothing and erases the highlight it was refreshing), and the
...
> **`bH8FJtL3F3` and `blobz/orchard` were removed from this set on 2026-08-01.** Both were listed as
> *"a decoy cage among honest ones"* and neither is one — see "The decoy-among-honest-cages category
> was never real" in [VALIDATORS.md](VALIDATORS.md). Do not re-add them as decoy tests.
...
1. **Cue recall** — recall, misses by source, and **false positives** (cue fired, no tag). Baselines
   ...
   94.4%, entropic 88.9%; nabner 100.0% (v3.152); ten line 96.6% (v3.153 — the single miss,
   `LPMhrPLMDQ`, is dropped on purpose by `TEN_LINE_ANTI_RE`).
