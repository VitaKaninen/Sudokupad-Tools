...
- **Settings clarity (what does this control do?):** Per-**control** 👁 hover icons (`makeHiliteIcon(key, title)`) sit right after each control's label (passed via `opts.hilite`/`hiliteTitle` on `buildSection`, `makeRangeRow`, `makeColorRow`, the Region-borders `makeSubsection`, and the action-section checkbox rows). `key` indexes four central maps: **HT** (what to highlight), **PAINT** (fill/stroke glow override; default per-element inference), **BLINK** (`{force,toggle}` enable-keys the click blinks), **ONSHOW** (optional hover "example"). Hover → `spdrHi.show(HT[key](), PAINT[key])`; click → `blinkEffect(BLINK[key])` (toggles the real effect on/off twice, no save). `spdrHi.dimPanel` fades `#sp-fix-panel` to 0.25 while a target sits under it.
  - **Renderer traces real geometry** (not bboxes): clones each svg target into a full-window overlay `<svg>` (`#spdr-highlight-layer` > osvg, viewBox `0 0 W H`, preserveAspectRatio none) positioned by `el.getScreenCTM()`. `inferPaint`/`styleNode`: filled shapes glow their FILL, lines/outlines glow their STROKE (so a colored border-strip glows itself, not a box around it). Text → bbox rect. Non-svg targets (tool/action buttons) → fixed outline div pool. `spdrHi.addText` draws overlay-only example digits.
  - **HT getters** mirror the apply predicates (FOOLPROOF PRINCIPLE): `objFillSources`/`objLineSources` = the exact sets `fixAllUnderlays`/`fixAllCagePaths`/`fixAllLines` touch; routing matches `computeObjectShade` (lines under colored/gray by stroke colour; Border = shape outlines only). `regCenter`/`regMulti`/`regCell` prefer injected clones (`data-spdr-kind`, set in `drawRegionSplitBorders`) else the live source geometry the draw clones from. `actionBtns`/`easyShade` → the on-screen buttons.
  - **ONSHOW examples** (overlay-only / transient, never edit the puzzle model): `centerValid`/`cornerValid` → `drawPencilExample` writes demo digits in empty cells (`emptyExampleCells`+`cellCenterScreen`); `selection` → `selectExampleCells` (real `app.select`, left selected on purpose).
  - Section **empty-state hints** unchanged: `HILITE[enabledKey]`=`sel`+`msg`, `spdrEmptyCheckers`/`refreshEmptyHints` on panel open. Tool-button ids `#control-colour`/`#control-centre`/`#control-corner`; action buttons `#sp-fill-btn-wrap`/`#sp-clear-btn-wrap`/`#sp-clearall-btn-wrap`, `#sp-easy-shade-btn`.
