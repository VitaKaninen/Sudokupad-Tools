...
- **Shaded-region colouring** (v2.123+) — recolour a puzzle's grey "extra regions" (`#cages path.cage-extraregion`, e.g. "grey regions must contain 1-9") with the 4-colour palette, adjacency-aware so touching regions differ. Drawn (v2.124) as recoloured path clones inside `mainGroup`, **below the region borders**; the original `#cages` paths are made transparent. The Easy Shade button cycles 4 states on puzzles that have them: off → Both (regions + shaded) → Regions → Shaded → off; the opacity card shows a 2nd "Shaded opacity" slider only on such puzzles, and a Both/Regions/Shaded indicator appears on the button. Normal puzzles keep the plain on/off toggle + single slider.
...
- **Shaded regions:** `assignExtraRegionColors` (samples each `cage-extraregion` path's cells via `isPointInFill` at cell centres, builds touch-adjacency, greedy 4-colours, stores idx in `dataset.spdrExtraColorIdx`), `applyExtraRegionFill`/`isExtraRegionColored` (read by `fixCagePath` — when active it makes the original `#cages` path **transparent**, because the colour is drawn elsewhere), `puzzleHasShadedRegions` (gates the button's 4-state cycle + 2nd slider). The colour itself is drawn by `drawRegionSplitBorders`: when `needShaded`, it calls `assignExtraRegionColors` again (observers trigger it decoupled from `applySettings`, so idx must be recomputed) and inserts recoloured path clones as the FIRST children of `mainGroup` → below the border strips. Button cycle lives in `buildEasyRegionShadeButton`.
