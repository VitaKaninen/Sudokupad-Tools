...
- **Cell shading** + **Object shading / underlay** — opacity / brightness controls. **Gray vs colored split (v2.140):** gray objects (saturation ≈ 0) read dim where colored objects read bright at the same slider value, so they have separate controls — colored objects use **Color brightness** + **Color opacity** (two sliders), gray objects use one linked **Gray brightness + opacity** slider (sets both axes to the same value). `computeObjectShade(c)` is the single router (gray detected by `isGrayColor`); colored brightness still uses the `absolute` mapping (pure hue at HSL lightness = slider) via `shadingTransform`. (The v2.130 `relative`/`luminance` brightness-mode selector was removed in v2.134 — only `absolute` survived; the `underlayBrightnessModel` setting is gone. All three default to 0.4 except Border brightness 0.5.) Lines (all stroked `#arrows` paths) and fills both route through `computeObjectShade`. It also shades coloured **`#overlay` shapes** (lucky-charm circles/squares/diamonds), not just `#underlay` — **always on** as of v2.140 (the `underlayOverlayEnabled` opt-in checkbox was removed), with a broad skip-rule (`shouldShadeOverlayRect`: skip `.textbg`, transparent, and white/near-white rects). Shape **outlines** keep their own independent **Border brightness** slider (`applyShapeStroke`, opacity locked at 1) — not part of the gray/colored split.
