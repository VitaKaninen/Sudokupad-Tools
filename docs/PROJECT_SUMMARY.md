...
- **Shaded regions:** `assignExtraRegionColors` (samples each `cage-extraregion` path's cells via `isPointInFill` at cell centres, builds touch-adjacency, greedy 4-colours, stores idx in `dataset.spdrExtraColorIdx`), `applyExtraRegionFill`/`isExtraRegionColored` (read by `fixCagePath` so the colour re-asserts on DR rewrites), `puzzleHasShadedRegions` (gates the button's 4-state cycle + 2nd slider). Wired into `applySettings` and the DR full-scan before `fixAllCagePaths`. Button cycle lives in `buildEasyRegionShadeButton`.
