...
- **Settings clarity (what does this section do?):** `HILITE` map (per-`enabledKey`: board `sel`, optional tool-button `control` fallback, empty-state `msg`) drives three features in `buildSection`. `hiliteTargets` reads the live elements a section affects. `spdrHi` (show/hide/dimPanel) pulses a fixed-position outline overlay around them on section hover — dims the settings panel (`#sp-fix-panel`) to ~0.18 opacity when a target sits under it. `flashSection` briefly toggles a section's effect on/off (no save) via the 👁 header button. Empty-state hints (`spdrEmptyCheckers`/`refreshEmptyHints`, run on panel open) show an amber ⚠ line when `sel` matches nothing. Tool-button ids: `#control-colour`/`#control-centre`/`#control-corner`.
