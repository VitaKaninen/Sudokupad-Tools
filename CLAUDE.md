...
- [`docs/VALIDATOR_POLICY.md`](docs/VALIDATOR_POLICY.md) — **the intended design for when a validator may speak, stay silent, or disappear.** Where the code disagrees with it, the code is wrong (v3.182–v3.186 built close to the opposite). Read it before any change to muting, menu-row state, or result reporting.
- [`docs/VALIDATORS.md`](docs/VALIDATORS.md) — the Validate-Constraints subsystem in full: architecture, per-validator notes, detection layers, ambiguity policy, the candidate-elimination contract. Read it before touching any validator. (For *policy* questions `VALIDATOR_POLICY.md` overrides it.)
...
**Finding code:** the script is one ~12,000-line IIFE with ~370 top-level functions. Don't read the whole file — grep for the function name in the "Code map" (PROJECT_SUMMARY) and read only that region.

**Any change to a validator — new one, bug fix, or behaviour tweak — starts by reading [`docs/VALIDATOR_POLICY.md`](docs/VALIDATOR_POLICY.md), and ends with `node tools/validator_harness.mjs` green before committing.** Not optional and not just for new validators: the policy governs muting, menu-row state, and how results are reported, which is most of what a "small fix" touches. A condensed five-rule version sits at the top of the in-code **ADDING A VALIDATOR** banner. The rollout that brought the shipped code in line landed across v3.188–v3.192 (its work-order file is deleted; the test set it used moved to `PROJECT_SUMMARY.md`).

**Two harness cases exist purely to stop that rollout being undone** — they assert `muteSolutionRefuted` and the whole-puzzle cage gate stay deleted. If one goes red, the fix is never to restore the code: re-read the policy first.
...
**The sync hook always fires once mid-bump — that is expected, not a failure.** It runs after *every* edit, and the two version markers live in separate strings, so after the first edit the file genuinely is mismatched and the hook reports a blocking error. Nothing is wrong: make the second edit immediately and the error is resolved. Don't treat the first error as a rejected edit and don't re-apply it (both edits land). Order and parallel-vs-sequential make no difference.
