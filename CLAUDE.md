...
### Version bumps + commit/push
Moved to the shared **[`../CLAUDE.md`](../CLAUDE.md)** (applies to all userscripts under
`Monkey Scripts/`): bump `@version` on every change, then `git add`/`commit`/`push` when finished.
This project's versions are semver minor increments (e.g. 2.119.0 → 2.120.0).
