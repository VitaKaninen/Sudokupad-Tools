...
**Comparing against the old (main-branch / DarkReader) script → ask for a live flip, inline.** When matching the native script to the old DR version (exact colours, spacing, etc.), the old version's look is DarkReader-generated and can only be read off a live old-script tab. When you hit that need **mid-task**, don't end the turn with a big handoff summary and don't try to toggle TamperMonkey/DarkReader yourself (the script-enable + DR state are entangled in the user's extension setup). Instead: (1) ask the user — in the middle of your reasoning — to flip your working tab to the old script and reload; (2) end the turn on just that short ask; (3) when they say it's done, immediately inspect the page and capture every value you need in one pass (don't make them flip back and forth); (4) then prompt them to flip it back to the native script. Keep it a tight ask → inspect → ask-back loop so the context window isn't re-read from scratch. (Both scripts run with DarkReader present; `window.spdrEdition` is `'native'` vs absent, and `window.spdrVersion` tells you 2.x vs 3.x.)
