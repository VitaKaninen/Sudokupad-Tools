...
**Any change to a validator — new one, bug fix, or behaviour tweak — starts by reading [`docs/VALIDATOR_POLICY.md`](docs/VALIDATOR_POLICY.md), and ends with `node tools/validator_harness.mjs` green before committing.** Not optional and not just for new validators: the policy governs muting, menu-row state, and how results are reported, which is most of what a "small fix" touches. A condensed five-rule version sits at the top of the in-code **ADDING A VALIDATOR** banner. ⚠️ *A rollout to bring the existing validators in line is in progress — see [`docs/VALIDATOR_POLICY_ROLLOUT.md`](docs/VALIDATOR_POLICY_ROLLOUT.md), which also lists the traps that would silently revert it. Delete that file when its last phase lands.*
