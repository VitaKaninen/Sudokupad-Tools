...
**Needing the user to swap tab state (flip script / toggle DR / toggle DMA) → ALWAYS request it via the `AskUserQuestion` popup, NEVER a prose turn-ending hand-back.** This is the hard rule (the user has insisted on it repeatedly): a plain prose "please flip it and tell me when done" ends the turn into a fresh context re-read and is treated as wasted tokens. Instead, when you hit the need mid-work, fire an `AskUserQuestion` like "I need the tab on <state> — make that switch now?" with one option "Yes — done, continue" (plus the write-in). The user flips it and answers in the popup, and you continue **in the same flow** without re-reading context. Apply this to every state swap: native↔main script, DarkReader on/off, dark-mode-alpha on/off. Do the swap-dependent inspection immediately on continuation and grab **everything** you need in one pass — never bounce them back and forth.

**Otherwise prefer a screenshot; flip only for pixel values.** When matching the native script to the old DR version, the old look is DarkReader-generated and only exists on a live old-script tab. **Default to asking (via the popup) for a screenshot of the state you need** — async, no tab-driving. Only request a live **flip + inspect** when you genuinely need *computed values* (exact `rgb()`, border widths) a screenshot can't give. Don't toggle TamperMonkey/DarkReader yourself (script-enable + DR state are entangled in the user's setup). (Both scripts run with DarkReader present; `window.spdrEdition` is `'native'` vs absent, `window.spdrVersion` tells 2.x vs 3.x.)
