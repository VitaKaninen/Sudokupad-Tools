...
**Comparing against the old (main-branch / DarkReader) script — prefer a screenshot; flip only for pixel values.** When matching the native script to the old DR version, the old look is DarkReader-generated and only exists on a live old-script tab. **Default to asking the user for a screenshot of the state you need** — that's async, needs no tab-driving, and costs no round-trip on your end. Only ask for a live **flip + inspect** when you genuinely need *computed values* (exact `rgb()`, border widths) a screenshot can't give. When you do flip: don't toggle TamperMonkey/DarkReader yourself (script-enable + DR state are entangled in the user's setup), keep the request to a single short ask, and when they say "done" grab **every** value you'll need in one pass — never make them flip back and forth, and never burn a whole turn of other work and *then* tack on a flip request. Batch all flip-requiring needs into that one cycle. (Both scripts run with DarkReader present; `window.spdrEdition` is `'native'` vs absent, `window.spdrVersion` tells 2.x vs 3.x.)
