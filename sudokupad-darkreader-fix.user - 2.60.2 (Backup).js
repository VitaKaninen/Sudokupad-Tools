...
(function () {
  ...
  // Debug test bench. Isolates the "select cells with conflicting digit X"
  // step from the rest of the remove pipeline so we can see exactly what
  // SudokuPad does with our synthetic events. Tries selection mechanisms one
  // at a time and reports detailed step-by-step results to a string array
  // (shown in the settings panel) and to the DevTools console.
  //
  // What it does NOT do: it does not click any digit button, so it never
  // modifies marks — safe to run repeatedly on any puzzle state.
  async function runMultiSelectTest(digit, mode) {
    var lines = [];
    function log(s) { lines.push(s); }
    var svg = document.getElementById('svgrenderer');
    console.group('[multi-select test] digit=' + digit + ' mode=' + mode);
    log('=== Multi-select test: digit "' + digit + '" mode "' + mode + '" ===');
    if (!svg) { log('FATAL: no #svgrenderer'); console.groupEnd(); return lines; }

    // ── Step 1: Scan DOM for conflict marks of this digit in this mode ─────
    var targets = [];
    var conflictSelector = mode === 'centre' ? '#cell-candidates tspan.conflict' : '#cell-pencilmarks text.conflict';
    document.querySelectorAll(conflictSelector).forEach(function (t) {
      if (t.getAttribute('data-val') !== digit) return;
      var srcEl = mode === 'centre' ? t.parentNode : t;
      var x = srcEl.getAttribute('x'), y = srcEl.getAttribute('y');
      targets.push({ cellX: x, cellY: y, digit: digit });
    });
    var targetCellKeys = targets.map(function (t) { return cellKeyFromMarkXY(t.cellX, t.cellY); });
    log('Step 1 — DOM scan');
    log('  Found ' + targets.length + ' conflict mark(s) for digit "' + digit + '" in ' + mode + ' mode');
    log('  Target cells: [' + targetCellKeys.join(' ') + ']');
    log('  Raw target xs/ys: ' + JSON.stringify(targets.map(function (t) { return [t.cellX, t.cellY]; })));
    console.log('targets:', targets);
    if (targets.length === 0) {
      log('(nothing to select — try a different digit)');
      console.groupEnd();
      return lines;
    }
    ...
    log('Step 2 — ensureMode("' + mode + '"): ' + (modeOk ? 'OK' : 'FAILED') + ' (current=' + getCurrentMode() + ')');
    if (!modeOk) { console.groupEnd(); return lines; }

    // ── Step 3: Pre-check — what's currently selected? ─────────────────────
    var initialSel = getSelectedCells();
    log('Step 3 — initial selection: ' + initialSel.size + ' cell(s) [' + Array.from(initialSel).join(' ') + ']');

    // ── Step 4: Try clearing with Escape ───────────────────────────────────
    document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', code: 'Escape', bubbles: true, cancelable: true }));
    document.dispatchEvent(new KeyboardEvent('keyup',   { key: 'Escape', code: 'Escape', bubbles: true, cancelable: true }));
    await sleep(30);
    var afterEsc = getSelectedCells();
    log('Step 4 — after Escape: ' + afterEsc.size + ' cell(s) [' + Array.from(afterEsc).join(' ') + ']');
    log('         Escape cleared selection: ' + (afterEsc.size === 0 ? 'YES' : 'NO ← problem'));

    // ── Step 5: Attempt A — drag-select (pointerdown→moves→pointerup) ──────
    log('Step 5 — Attempt A: drag-select (single pointerdown→pointermoves→pointerup)');
    var points = targets.map(function (t) {
      var cx = parseFloat(t.cellX), cy = parseFloat(t.cellY);
      return svgToClient(svg, cx, cy);
    });
    dispatchCellDrag(points);
    await sleep(50);
    var afterDrag = getSelectedCells();
    log('  Selected after drag: ' + afterDrag.size + ' cell(s) [' + Array.from(afterDrag).join(' ') + ']');
    var dragExpected = new Set(targetCellKeys);
    var dragExtras = Array.from(afterDrag).filter(function (k) { return !dragExpected.has(k); });
    var dragMissing = targetCellKeys.filter(function (k) { return !afterDrag.has(k); });
    log('  Extras (selected but not a target): [' + dragExtras.join(' ') + ']');
    log('  Missing (target but not selected): [' + dragMissing.join(' ') + ']');
    log('  Match: ' + (dragExtras.length === 0 && dragMissing.length === 0 ? 'YES' : 'NO'));
    if (dragExtras.length > 0) {
      log('  Extras safety analysis (a click is safe on extras that have a value/given):');
      var allExtrasSafe = true;
      dragExtras.forEach(function (k) {
        var parts = k.split(',').map(Number);
        var hasVal = cellHasValueOrGiven(parts[0], parts[1]);
        log('    ' + k + ': hasValueOrGiven=' + hasVal + ' → ' + (hasVal ? 'SAFE' : 'UNSAFE'));
        if (!hasVal) allExtrasSafe = false;
      });
      log('  All extras safe: ' + (allExtrasSafe ? 'YES — drag-then-click would be correct' : 'NO — would modify extras'));
    }

    // Cleanup before next attempt: switch to normal+click somewhere safe to reset,
    // then back to test mode. (Avoids relying on Escape if it doesn't clear multis.)
    await ensureMode('normal');
    var firstPt = svgToClient(svg, parseFloat(targets[0].cellX), parseFloat(targets[0].cellY));
    dispatchClickAt(firstPt.x, firstPt.y);
    await sleep(20);
    document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', code: 'Escape', bubbles: true, cancelable: true }));
    document.dispatchEvent(new KeyboardEvent('keyup',   { key: 'Escape', code: 'Escape', bubbles: true, cancelable: true }));
    await sleep(20);
    await ensureMode(mode);
    var afterReset = getSelectedCells();
    log('  Cleanup before next attempt: ' + afterReset.size + ' cell(s) selected (want 0)');

    // ── Step 6: Attempt B — sequential clicks in target mode (no Ctrl) ─────
    log('Step 6 — Attempt B: ' + targets.length + ' sequential clicks in ' + mode + ' mode (no modifier)');
    ...
    await sleep(30);
    var afterClicks = getSelectedCells();
    log('  Selected after clicks: ' + afterClicks.size + ' cell(s) [' + Array.from(afterClicks).join(' ') + ']');
    var clickExtras = Array.from(afterClicks).filter(function (k) { return !dragExpected.has(k); });
    var clickMissing = targetCellKeys.filter(function (k) { return !afterClicks.has(k); });
    log('  Extras: [' + clickExtras.join(' ') + ']');
    log('  Missing: [' + clickMissing.join(' ') + ']');
    log('  Match: ' + (clickExtras.length === 0 && clickMissing.length === 0 ? 'YES' : 'NO'));
    ...
    // Helper: dispatch a click sequence at (x,y) with arbitrary modifiers on the events.
    function dispatchModClick(grid, x, y, mods) {
      var initDown = Object.assign({
        bubbles: true, cancelable: true, view: window,
        clientX: x, clientY: y,
        button: 0, buttons: 1, isPrimary: true, pointerType: 'mouse',
      }, mods);
      var initUp = Object.assign({}, initDown, { buttons: 0 });
      grid.dispatchEvent(new PointerEvent('pointerdown', initDown));
      grid.dispatchEvent(new MouseEvent('mousedown',     initDown));
      grid.dispatchEvent(new PointerEvent('pointerup',   initUp));
      grid.dispatchEvent(new MouseEvent('mouseup',       initUp));
      grid.dispatchEvent(new MouseEvent('click',         initUp));
    }
    var grid = document.querySelector('.grid');

    // ── Step 7: Attempt C — sequential clicks with ctrlKey only (no metaKey) ─
    log('Step 7 — Attempt C: ' + targets.length + ' sequential clicks with ctrlKey:true (event property only)');
    for (var j = 0; j < targets.length; j++) {
      if (!grid) break;
      dispatchModClick(grid, points[j].x, points[j].y, { ctrlKey: true });
      await sleep(15);
    }
    await sleep(30);
    var afterCtrlClicks = getSelectedCells();
    log('  Selected after Ctrl+clicks: ' + afterCtrlClicks.size + ' cell(s) [' + Array.from(afterCtrlClicks).join(' ') + ']');
    var ctrlExtras = Array.from(afterCtrlClicks).filter(function (k) { return !dragExpected.has(k); });
    var ctrlMissing = targetCellKeys.filter(function (k) { return !afterCtrlClicks.has(k); });
    log('  Extras: [' + ctrlExtras.join(' ') + ']');
    log('  Missing: [' + ctrlMissing.join(' ') + ']');
    log('  Match: ' + (ctrlExtras.length === 0 && ctrlMissing.length === 0 ? 'YES' : 'NO'));
    ...
    // ── Step 7b: Attempt E — Ctrl+click WITH keydown/keyup wrapping ────────
    // Same as C but also dispatches Control-key down BEFORE the clicks and up
    // AFTER. If SudokuPad tracks modifier state via document-level key events
    // (rather than reading event.ctrlKey on each click), this version will
    // register as a real Ctrl+click and the previous one won't.
    log('Step 7b — Attempt E: same with keydown(Control)…keyup(Control) wrapper');
    ...
    for (var k = 0; k < targets.length; k++) {
      if (!grid) break;
      dispatchModClick(grid, points[k].x, points[k].y, { ctrlKey: true });
      await sleep(15);
    }
    await sleep(30);
    var afterCtrlHold = getSelectedCells();
    document.dispatchEvent(new KeyboardEvent('keyup', {
      key: 'Control', code: 'ControlLeft', keyCode: 17, which: 17,
      ctrlKey: false, bubbles: true, cancelable: true,
    }));
    await sleep(10);
    log('  Selected after Ctrl-held clicks: ' + afterCtrlHold.size + ' cell(s) [' + Array.from(afterCtrlHold).join(' ') + ']');
    var ctrlHoldExtras = Array.from(afterCtrlHold).filter(function (k) { return !dragExpected.has(k); });
    var ctrlHoldMissing = targetCellKeys.filter(function (k) { return !afterCtrlHold.has(k); });
    log('  Extras: [' + ctrlHoldExtras.join(' ') + ']');
    log('  Missing: [' + ctrlHoldMissing.join(' ') + ']');
    log('  Match: ' + (ctrlHoldExtras.length === 0 && ctrlHoldMissing.length === 0 ? 'YES' : 'NO'));
    ...
    // ── Step 7c: Attempt F — Shift+click with keydown/keyup wrapping ───────
    log('Step 7c — Attempt F: sequential clicks with shiftKey + keydown(Shift) wrapper');
    document.dispatchEvent(new KeyboardEvent('keydown', {
    ...
    for (var m = 0; m < targets.length; m++) {
      if (!grid) break;
      dispatchModClick(grid, points[m].x, points[m].y, { shiftKey: true });
      await sleep(15);
    }
    await sleep(30);
    var afterShift = getSelectedCells();
    document.dispatchEvent(new KeyboardEvent('keyup', {
      key: 'Shift', code: 'ShiftLeft', keyCode: 16, which: 16,
      shiftKey: false, bubbles: true, cancelable: true,
    }));
    await sleep(10);
    log('  Selected after Shift-held clicks: ' + afterShift.size + ' cell(s) [' + Array.from(afterShift).join(' ') + ']');
    var shiftExtras = Array.from(afterShift).filter(function (k) { return !dragExpected.has(k); });
    var shiftMissing = targetCellKeys.filter(function (k) { return !afterShift.has(k); });
    log('  Extras: [' + shiftExtras.join(' ') + ']');
    log('  Missing: [' + shiftMissing.join(' ') + ']');
    log('  Match: ' + (shiftExtras.length === 0 && shiftMissing.length === 0 ? 'YES' : 'NO'));

    // Cleanup before final attempt
    await ensureMode('normal');
    dispatchClickAt(firstPt.x, firstPt.y);
    await sleep(20);
    document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', code: 'Escape', bubbles: true, cancelable: true }));
    ...
    // ── Step 8: Attempt D — drag + actual digit click + auto-undo ──────────
    // Confirms the hypothesis: if drag-then-click produces a diff with only
    // the expected removals (no changes for extras), the drag is safe even
    // when extras are present (as long as those extras have values/givens).
    log('Step 8 — Attempt D: drag + click "' + digit + '" + auto-undo');
    var digitBtn = getDigitButton(digit);
    if (!digitBtn) {
      log('  No digit button for "' + digit + '" — skipping click test');
    } else {
      dispatchCellDrag(points);
      await sleep(50);
      var preClickSel = getSelectedCells();
      log('  Selected before click: ' + preClickSel.size + ' cell(s)');
      var beforeClick = snapshotPencilmarks();
      dispatchClickEl(digitBtn);
      await sleep(50);
      var afterClick = snapshotPencilmarks();
      var diff = diffSnapshots(beforeClick, afterClick);
      var expectedKeys = targets.map(markKey);
      log('  Expected removals (' + mode + '): [' + expectedKeys.join(' ') + ']');
      log('  Actual diff.removed.' + mode + ' (' + diff.removed[mode].length + '): [' + diff.removed[mode].join(' ') + ']');
      log('  Actual diff.added.' + mode + ' (' + diff.added[mode].length + '): [' + diff.added[mode].join(' ') + ']');
      var otherMode = mode === 'corner' ? 'centre' : 'corner';
      log('  Actual diff.removed.' + otherMode + ' (' + diff.removed[otherMode].length + '): [' + diff.removed[otherMode].join(' ') + ']');
      log('  Actual diff.added.' + otherMode + ' (' + diff.added[otherMode].length + '): [' + diff.added[otherMode].join(' ') + ']');
      log('  diff.removed.values: ' + JSON.stringify(diff.removed.values));
      log('  diff.added.values: ' + JSON.stringify(diff.added.values));
      var expectedSet = new Set(expectedKeys);
      var actualSet   = new Set(diff.removed[mode]);
      var matchesExpected = (
        diff.added.corner.length === 0 && diff.added.centre.length === 0 &&
        diff.added.values.length === 0 && diff.added.colors.length === 0 &&
        diff.removed[otherMode].length === 0 &&
        diff.removed.values.length === 0 && diff.removed.colors.length === 0 &&
        actualSet.size === expectedSet.size &&
        expectedKeys.every(function (k) { return actualSet.has(k); })
      );
      log('  Diff matches expected (exactly the target removals, nothing else): ' + (matchesExpected ? 'YES' : 'NO'));

      // Auto-undo to restore puzzle
      var undoBtn = getModeButton('undo');
      if (undoBtn) {
        dispatchClickEl(undoBtn);
        await sleep(150);
        var afterUndo = snapshotPencilmarks();
        var diffUndo  = diffSnapshots(beforeClick, afterUndo);
        log('  After undo: ' + (diffEmpty(diffUndo) ? 'fully restored ✓' : 'differences remain (might need extra undo)'));
      } else {
        log('  WARNING: no undo button found — puzzle may be left in modified state!');
      }
    }

    // Cleanup before the modifier-drag tests
    await ensureMode('normal');
    dispatchClickAt(firstPt.x, firstPt.y);
    await sleep(20);
    document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', code: 'Escape', bubbles: true, cancelable: true }));
    ...
    // Helper: dispatch a drag through `pointsList` with arbitrary modifiers
    // (ctrlKey / shiftKey / metaKey) attached to every pointer/mouse event.
    function dispatchDragWithMods(pointsList, mods) {
      if (!pointsList || pointsList.length === 0) return;
      var g = document.querySelector('.grid');
      if (!g) return;
      function evtInit(p, buttons) {
      ...
      g.dispatchEvent(new PointerEvent('pointerdown', evtInit(pointsList[0], 1)));
      g.dispatchEvent(new MouseEvent('mousedown',     evtInit(pointsList[0], 1)));
      for (var i = 1; i < pointsList.length; i++) {
        g.dispatchEvent(new PointerEvent('pointermove', evtInit(pointsList[i], 1)));
        g.dispatchEvent(new MouseEvent('mousemove',     evtInit(pointsList[i], 1)));
      }
      var last = pointsList[pointsList.length - 1];
      g.dispatchEvent(new PointerEvent('pointerup', evtInit(last, 0)));
      g.dispatchEvent(new MouseEvent('mouseup',     evtInit(last, 0)));
      // No click event for multi-point — real drag never fires click after move.
    }

    // ── Step 9: Attempt G — Ctrl+drag as ADDITIVE multi-select ─────────────
    // Real test for non-contiguous multi-select: split targets into two halves,
    // drag the first half (plain), then Ctrl+drag the second half. If the
    // final selection contains BOTH halves, Ctrl+drag adds to selection and
    // we can use it for arbitrary non-contiguous batches.
    if (points.length >= 2) {
      log('Step 9 — Attempt G: plain drag first half + Ctrl+drag second half (additive multi-select)');
      var mid = Math.floor(points.length / 2);
      var firstHalf  = points.slice(0, mid);
      var secondHalf = points.slice(mid);
      var firstHalfKeys  = targetCellKeys.slice(0, mid);
      var secondHalfKeys = targetCellKeys.slice(mid);

      dispatchCellDrag(firstHalf);
      await sleep(50);
      var afterFirst = getSelectedCells();
      log('  After plain drag of ' + firstHalf.length + ' cells: ' + afterFirst.size + ' selected');

      // Hold Ctrl down for the additive drag
      document.dispatchEvent(new KeyboardEvent('keydown', {
        key: 'Control', code: 'ControlLeft', keyCode: 17, which: 17,
        ctrlKey: true, bubbles: true, cancelable: true,
      }));
      await sleep(10);
      dispatchDragWithMods(secondHalf, { ctrlKey: true });
      await sleep(50);
      var afterCtrlDrag = getSelectedCells();
      document.dispatchEvent(new KeyboardEvent('keyup', {
        key: 'Control', code: 'ControlLeft', keyCode: 17, which: 17,
        ctrlKey: false, bubbles: true, cancelable: true,
      }));
      await sleep(10);
      log('  After Ctrl+drag of ' + secondHalf.length + ' cells: ' + afterCtrlDrag.size + ' selected [' + Array.from(afterCtrlDrag).join(' ') + ']');

      var firstHalfStillIn  = firstHalfKeys.every(function (k) { return afterCtrlDrag.has(k); });
      var secondHalfNowIn   = secondHalfKeys.every(function (k) { return afterCtrlDrag.has(k); });
      if (firstHalfStillIn && secondHalfNowIn) {
        log('  Verdict: Ctrl+drag ADDS to selection ✓ — non-contiguous multi-select is feasible!');
      } else if (secondHalfNowIn && !firstHalfStillIn) {
        log('  Verdict: Ctrl+drag REPLACED selection (only second half selected). Additive multi-select not available via Ctrl+drag.');
      } else if (firstHalfStillIn && !secondHalfNowIn) {
        log('  Verdict: Ctrl+drag had no effect (only first half still selected).');
      } else {
        log('  Verdict: Ctrl+drag produced unexpected state — neither full additive nor pure replace.');
      }
    }

    // Cleanup before Attempt H
    await ensureMode('normal');
    dispatchClickAt(firstPt.x, firstPt.y);
    await sleep(20);
    document.dispatchEvent(new KeyboardEvent('keydown', { key: 'Escape', code: 'Escape', bubbles: true, cancelable: true }));
    ...
    // ── Step 10: Attempt H — Shift+drag (in case Shift is the additive key) ─
    if (points.length >= 2) {
      log('Step 10 — Attempt H: plain drag first half + Shift+drag second half');
      var midH = Math.floor(points.length / 2);
      var firstH  = points.slice(0, midH);
      var secondH = points.slice(midH);
      var firstHKeys  = targetCellKeys.slice(0, midH);
      var secondHKeys = targetCellKeys.slice(midH);

      dispatchCellDrag(firstH);
      await sleep(50);
      log('  After plain drag: ' + getSelectedCells().size + ' selected');
      ...
      await sleep(10);
      dispatchDragWithMods(secondH, { shiftKey: true });
      await sleep(50);
      var afterShiftDrag = getSelectedCells();
      document.dispatchEvent(new KeyboardEvent('keyup', {
        key: 'Shift', code: 'ShiftLeft', keyCode: 16, which: 16,
        shiftKey: false, bubbles: true, cancelable: true,
      }));
      await sleep(10);
      log('  After Shift+drag: ' + afterShiftDrag.size + ' selected');

      var firstStillIn  = firstHKeys.every(function (k) { return afterShiftDrag.has(k); });
      var secondNowIn   = secondHKeys.every(function (k) { return afterShiftDrag.has(k); });
      if (firstStillIn && secondNowIn) {
        log('  Verdict: Shift+drag ADDS to selection ✓');
      } else if (secondNowIn && !firstStillIn) {
        log('  Verdict: Shift+drag REPLACED selection.');
      } else {
        log('  Verdict: Shift+drag produced unexpected state.');
      }
    }
  ...
  function buildSettingsUI() {
    ...
    Object.assign(digitSetRow.style, { marginTop: '10px', paddingTop: '8px', borderTop: '1px solid #313244' });
    var digitSetLabelRow = document.createElement('div');
    digitSetLabelRow.style.cssText = 'display:flex;justify-content:space-between;align-items:baseline;margin-bottom:4px;';
    var digitSetLabel = document.createElement('span');
    digitSetLabel.textContent = 'Digits used in this puzzle:';
    digitSetLabel.style.cssText = 'font-size:11px;color:#a6adc8;';
    var rescanLink = document.createElement('button');
    rescanLink.type = 'button';
    rescanLink.textContent = 'Re-scan';
    rescanLink.style.cssText = 'font-size:10px;color:#a6adc8;background:none;border:none;padding:0;cursor:pointer;text-decoration:underline;';
    rescanLink.addEventListener('click', function () {
      settings.digitSetConfirmed = false;
      settings.lastConfirmedUrl = '';
      saveSettings(settings);
      runDigitSetCheck();
    });
    digitSetLabelRow.appendChild(digitSetLabel);
    digitSetLabelRow.appendChild(rescanLink);
    ...
    digitSetRow.appendChild(digitSetLabelRow);
    digitSetRow.appendChild(digitSetInput);
    actionSection.appendChild(digitSetRow);

    panel.appendChild(actionSection);

    // ── Debug section: multi-select test bench ─────────────────────────────────
    // Lets us isolate the "select cells with conflicting digit X" step from
    // everything else, and see exactly what SudokuPad does with our synthetic
    // events. Output appears in the box below the button AND in the DevTools
    // console with grouped logging.
    var debugSection = document.createElement('div');
    Object.assign(debugSection.style, { padding: '8px 14px', borderTop: '1px solid #45475a' });
    var debugTitle = document.createElement('div');
    debugTitle.textContent = 'Debug: multi-select test';
    Object.assign(debugTitle.style, { color: '#cdd6f4', fontSize: '11px', fontWeight: 'bold', marginBottom: '6px' });
    debugSection.appendChild(debugTitle);

    var debugControls = document.createElement('div');
    Object.assign(debugControls.style, { display: 'flex', gap: '6px', alignItems: 'center', marginBottom: '6px' });
    var dbgDigitLabel = document.createElement('span');
    dbgDigitLabel.textContent = 'Digit:';
    Object.assign(dbgDigitLabel.style, { color: '#a6adc8', fontSize: '11px' });
    var dbgDigitInput = document.createElement('input');
    dbgDigitInput.id = 'sp-dbg-digit';
    dbgDigitInput.type = 'text';
    dbgDigitInput.value = '2';
    Object.assign(dbgDigitInput.style, {
      width: '36px', background: '#181825', color: '#cdd6f4',
      border: '1px solid #45475a', borderRadius: '4px',
      padding: '2px 6px', fontSize: '11px', fontFamily: 'monospace', textAlign: 'center',
    });
    var dbgModeSelect = document.createElement('select');
    dbgModeSelect.id = 'sp-dbg-mode';
    ['centre', 'corner'].forEach(function (m) {
      var opt = document.createElement('option');
      opt.value = m; opt.textContent = m;
      dbgModeSelect.appendChild(opt);
    });
    Object.assign(dbgModeSelect.style, {
      background: '#181825', color: '#cdd6f4',
      border: '1px solid #45475a', borderRadius: '4px',
      padding: '2px 6px', fontSize: '11px',
    });
    var dbgRunBtn = document.createElement('button');
    dbgRunBtn.type = 'button';
    dbgRunBtn.textContent = 'Run select test';
    Object.assign(dbgRunBtn.style, {
      background: '#313244', color: '#cdd6f4',
      border: '1px solid #45475a', borderRadius: '4px',
      padding: '4px 10px', cursor: 'pointer', fontSize: '11px', flex: '1',
    });
    var dbgCopyBtn = document.createElement('button');
    dbgCopyBtn.type = 'button';
    dbgCopyBtn.textContent = 'Copy';
    dbgCopyBtn.title = 'Copy test output to clipboard';
    Object.assign(dbgCopyBtn.style, {
      background: '#313244', color: '#cdd6f4',
      border: '1px solid #45475a', borderRadius: '4px',
      padding: '4px 10px', cursor: 'pointer', fontSize: '11px',
    });
    debugControls.appendChild(dbgDigitLabel);
    debugControls.appendChild(dbgDigitInput);
    debugControls.appendChild(dbgModeSelect);
    debugControls.appendChild(dbgRunBtn);
    debugControls.appendChild(dbgCopyBtn);
    debugSection.appendChild(debugControls);

    var dbgOutput = document.createElement('pre');
    dbgOutput.id = 'sp-dbg-output';
    Object.assign(dbgOutput.style, {
      background: '#181825', color: '#cdd6f4',
      border: '1px solid #45475a', borderRadius: '4px',
      padding: '6px 8px', fontSize: '10px', fontFamily: 'monospace',
      maxHeight: '180px', overflow: 'auto', margin: '0',
      whiteSpace: 'pre-wrap', wordBreak: 'break-word',
      userSelect: 'text', WebkitUserSelect: 'text', MozUserSelect: 'text',
      cursor: 'text',
    });
    dbgOutput.textContent = '(no test run yet)';
    debugSection.appendChild(dbgOutput);

    dbgRunBtn.addEventListener('click', function () {
      var digit = (dbgDigitInput.value || '').trim().charAt(0);
      var mode  = dbgModeSelect.value;
      if (!digit) { dbgOutput.textContent = 'ERROR: enter a digit first'; return; }
      dbgRunBtn.disabled = true;
      dbgOutput.textContent = 'Running…';
      runMultiSelectTest(digit, mode).then(function (lines) {
        dbgOutput.textContent = lines.join('\n');
        dbgRunBtn.disabled = false;
      }).catch(function (e) {
        dbgOutput.textContent = 'TEST CRASHED: ' + (e && e.message || e);
        dbgRunBtn.disabled = false;
        console.error('[multi-select test] crashed:', e);
      });
    });

    dbgCopyBtn.addEventListener('click', function () {
      var text = dbgOutput.textContent || '';
      var done = function (ok) {
        var orig = dbgCopyBtn.textContent;
        dbgCopyBtn.textContent = ok ? 'Copied!' : 'Copy failed';
        setTimeout(function () { dbgCopyBtn.textContent = orig; }, 1200);
      };
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(text).then(function () { done(true); }, function () {
          // Fallback to execCommand if Clipboard API rejects (e.g. focus issues).
          fallbackCopy();
        });
      } else {
        fallbackCopy();
      ...
      function fallbackCopy() {
        ...
        ta.value = text;
        Object.assign(ta.style, { position: 'fixed', top: '-1000px', left: '-1000px', opacity: '0' });
